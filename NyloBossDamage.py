...
##--------------------Melee Nylo Boss Spec Simulation -----------------
# 
# 
# 
...
weapon_options = {
    '1': ('Dragon Claws', dragon_claws),
    '2': ('Crystal Halberd', crystal_halberd),
    '3': ('Scythe (regular hit)', scythe),
}

# Boss mapping for user input
boss_options = {
    '1': ('Nylo Boss', Nylo_boss),
}


def get_user_input():
    """
    Get user input for number of players and weapon specs.
    
    Returns:
        Tuple of (boss_class, num_players, player_specs)
    """
    print("\n" + "="*50)
    print("NYLO BOSS SPEC SIMULATION SETUP")
    print("="*50)
    
    # Set boss to Nylo Boss (no user selection needed)
    boss_name = 'Nylo Boss'
    ...
    # Nylo Boss has 2 rounds of specs
...
def run_defense_level_analysis(num_players, num_simulations=1000):
    """
    Test all weapon combinations across defense levels 0-50.
    
    Args:
        num_players: Number of players (3-5)
        num_simulations: Number of simulations per combination (default 1000)
    
    Returns:
        Dictionary with results for each defense level and weapon combination
    """
    weapons_list = list(weapon_options.values())
    
    print("\n" + "="*60)
    print("NYLO BOSS DEFENSE LEVEL ANALYSIS")
    print("="*60)
    print(f"Testing all weapon combinations from defense 0-50")
    print(f"Simulations per combination: {num_simulations}")
    print(f"Number of players: {num_players}")
    print()
    
    # Store results: {starting_def: {(weapon1, weapon2): avg_damage}}
    defense_results = {}
    
    # Test each starting defense level (0-50)
    for starting_def in range(0, 51):
        print(f"\nTesting starting defense: {starting_def}")
        defense_results[starting_def] = {}
        
        # Test all weapon combinations for round 1 and round 2
        # Only test unique combinations (w1_idx <= w2_idx to avoid duplicates)
        for w1_idx, (w1_name, w1_weapon) in enumerate(weapons_list):
            for w2_idx, (w2_name, w2_weapon) in enumerate(weapons_list):
                # Skip if we've already tested this combination in reverse
                if w2_idx < w1_idx:
                    continue
                
                combo_name = f"{w1_name} -> {w2_name}"
                
                # Create player_specs: all players use same weapon each round
                player_specs = [
                    [w1_weapon] * num_players,  # Round 1
                    [w2_weapon] * num_players   # Round 2
                ]
                
                # Run simulations with this combination at this defense level
                combo_damage = []
                ...
                for sim in range(num_simulations):
                    ...
                    # Create boss and set defense level
                    boss = Nylo_boss(num_players)
                    boss.stats.def_level = starting_def
                    
                    total_damage = 0
                    
                    # Round 1
                    for player in players:
                        player.equip_weapon(w1_weapon)
                        if w1_weapon.__class__.__name__ == 'Scythe':
                            hit = player.do_attack(boss, special_attack=False)
                        else:
                            hit = player.do_attack(boss, special_attack=True)
                        boss.reduce_hp(hit)
                    ...
                    # Round 2
                    for player in players:
                        player.equip_weapon(w2_weapon)
                        if w2_weapon.__class__.__name__ == 'Scythe':
                            hit = player.do_attack(boss, special_attack=False)
                        else:
                            hit = player.do_attack(boss, special_attack=True)
                        boss.reduce_hp(hit)
                    ...
                    combo_damage.append(total_damage)
                
                # Calculate average damage for this combination
                avg_damage = sum(combo_damage) / num_simulations
                avg_per_player = avg_damage / num_players
                defense_results[starting_def][combo_name] = avg_damage
                
                print(f"  {combo_name}: {avg_damage:.2f} (avg per player: {avg_per_player:.2f})")
    
    return defense_results


def print_defense_analysis_summary(defense_results, num_players):
    """
    Print a summary table of the defense level analysis.
    """
    print("\n" + "="*80)
    print("DEFENSE LEVEL ANALYSIS SUMMARY - TEAM DAMAGE")
    print("="*80)
    
    weapons_list = list(weapon_options.values())
    
    # Build list of unique combinations in order tested
    unique_combos = []
    for w1_idx, (w1_name, _) in enumerate(weapons_list):
        for w2_idx, (w2_name, _) in enumerate(weapons_list):
            if w2_idx >= w1_idx:  # Only include combinations we actually tested
                unique_combos.append((w1_name, w2_name))
    
    # Print team damage table
    print(f"\n{'Def':<5}", end="")
    for w1_name, w2_name in unique_combos:
        combo = f"{w1_name[:4]}->{w2_name[:4]}"
        print(f"{combo:<13}", end="")
    print()
    print("-" * (5 + 13 * len(unique_combos)))
    
    # Print each defense level row for team damage
    for def_lvl in sorted(defense_results.keys()):
        print(f"{def_lvl:<5}", end="")
        for w1_name, w2_name in unique_combos:
            combo_name = f"{w1_name} -> {w2_name}"
            if combo_name in defense_results[def_lvl]:
                avg_dmg = defense_results[def_lvl][combo_name]
                print(f"{avg_dmg:<13.1f}", end="")
            else:
                print(f"{'N/A':<13}", end="")
        print()
    
    # Print per-player damage table
    print("\n" + "="*80)
    print(f"DAMAGE PER PLAYER ({num_players} players)")
    print("="*80)
    ...
    # Print each defense level row for per-player damage
    for def_lvl in sorted(defense_results.keys()):
        print(f"{def_lvl:<5}", end="")
        for w1_name, w2_name in unique_combos:
            combo_name = f"{w1_name} -> {w2_name}"
            if combo_name in defense_results[def_lvl]:
                avg_dmg_per_player = defense_results[def_lvl][combo_name] / num_players
                print(f"{avg_dmg_per_player:<13.1f}", end="")
            else:
                print(f"{'N/A':<13}", end="")
        print()
...
print()
print("Running Nylo Boss Defense Level Analysis")
print("="*50)
...
# Ask for number of simulations per combination
while True:
    try:
        num_sims = int(input("\nHow many simulations per combination? (1-10000): "))
        if 1 <= num_sims <= 10000:
            break
        else:
...
# Calculate unique combinations: n*(n+1)/2 where n is number of weapons
num_weapons = len(weapon_options)
unique_combos = num_weapons * (num_weapons + 1) // 2

print(f"\nRunning defense level analysis...")
print(f"Total unique combinations: {unique_combos}")
print(f"Total simulations: {51 * unique_combos * num_sims:,}")

# Run the analysis
defense_results = run_defense_level_analysis(num_players, num_sims)

# Print summary table
print_defense_analysis_summary(defense_results, num_players)
